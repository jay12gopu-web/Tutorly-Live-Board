...
function ObjectShape({ object, selected, dragOffset }: { object: AnyBoardObject; selected: boolean; dragOffset: Point }) {
  ...
  const animationClass = object.animation ? `reveal-${object.animation.kind}` : '';
  const common = {
    className: `board-object ${object.createdBy} ${object.layer === 'TutorlyLayer' ? 'tutorly-layer' : 'student-layer'} ${animationClass} ${selected ? 'selected-object' : ''}`,
    stroke,
    strokeWidth: selected ? Math.max(4, (object.strokeWidth ?? 2.5) + 1.5) : object.strokeWidth ?? 2.5,
    opacity: object.opacity ?? 1,
    fill: 'none',
    style: object.animation ? { animationDuration: `${object.animation.duration ?? 900}ms`, animationDelay: `${object.animation.delay ?? 0}ms` } : undefined,
  };
...
export function BoardCanvas({ state, activeTool, selectedId, panMode, studentWorkMode, onSelect, onCommand, onViewport, onStudentAction, onTogglePan }: BoardCanvasProps) {
  ...
  const objectAt = (point: Point) => studentWorkMode ? [...state.objects].reverse().find((object) => object.createdBy === 'student' && hitTest(object, point)) : undefined;
  const finishDrawing = (point: Point) => {
    const current = draftRef.current;
    if (!current || !studentWorkMode) return;
    const { start } = current;
    const byStudent = { createdBy: 'student' as const };
    if (activeTool === 'pen' || activeTool === 'highlighter') {
      const points = current.points ?? [];
      if (points.length > 1) { onCommand(activeTool === 'highlighter' ? { type: 'highlight', payload: { points, ...byStudent } } : { type: 'draw_path', payload: { points, ...byStudent } }); onStudentAction(activeTool === 'highlighter' ? 'highlight' : 'path'); }
    } else if (activeTool === 'line' || activeTool === 'ray' || activeTool === 'arrow') {
      onCommand({ type: activeTool === 'line' ? 'draw_line' : activeTool === 'ray' ? 'draw_ray' : 'draw_arrow', payload: { x1: start.x, y1: start.y, x2: point.x, y2: point.y, ...byStudent } }); onStudentAction(activeTool);
    } else if (activeTool === 'circle') { onCommand({ type: 'draw_circle', payload: { cx: start.x, cy: start.y, r: Math.max(8, distance(start, point)), ...byStudent } }); onStudentAction(activeTool);
    } else if (activeTool === 'arc') { onCommand({ type: 'draw_arc', payload: { cx: start.x, cy: start.y, r: Math.max(8, distance(start, point)), startAngle: 0, endAngle: angleOf(start, point), ...byStudent } }); onStudentAction(activeTool);
    } else if (activeTool === 'rectangle') { onCommand({ type: 'draw_rectangle', payload: { x: Math.min(start.x, point.x), y: Math.min(start.y, point.y), width: Math.abs(point.x - start.x), height: Math.abs(point.y - start.y), ...byStudent } }); onStudentAction(activeTool); }
    draftRef.current = null; setDraft(null);
  };
  const onPointerDown = (event: PointerEvent<SVGSVGElement>) => {
    event.currentTarget.setPointerCapture(event.pointerId);
    const point = toBoardPoint(event);
    if (panMode) { panRef.current = { start: toSvgPoint(event), panX: state.viewport.panX, panY: state.viewport.panY }; return; }
    ...
    if (activeTool === 'axes' || activeTool === 'graph') { onCommand({ type: activeTool === 'axes' ? 'draw_axes' : 'draw_graph', payload: { createdBy: 'student' } }); onStudentAction(activeTool); return; }
    draftRef.current = { start: point, current: point, points: activeTool === 'pen' || activeTool === 'highlighter' ? [point] : undefined }; setDraft(draftRef.current);
  };
  const onPointerMove = (event: PointerEvent<SVGSVGElement>) => {
    const point = toBoardPoint(event);
    if (panRef.current) { const pan = panRef.current; const raw = toSvgPoint(event); onViewport({ ...state.viewport, panX: pan.panX + (raw.x - pan.start.x), panY: pan.panY + (raw.y - pan.start.y) }); return; }
    if (dragRef.current) { setDragOffset({ x: point.x - dragRef.current.start.x, y: point.y - dragRef.current.start.y }); return; }
    if (!draftRef.current) return;
    draftRef.current.current = point; if (draftRef.current.points) draftRef.current.points = [...draftRef.current.points, point]; setDraft({ ...draftRef.current, points: draftRef.current.points ? [...draftRef.current.points] : undefined });
  };
  const onPointerUp = (event: PointerEvent<SVGSVGElement>) => {
    const point = toBoardPoint(event);
    if (panRef.current) { panRef.current = null; return; }
    if (dragRef.current) { const drag = dragRef.current; if (Math.abs(point.x - drag.start.x) > 2 || Math.abs(point.y - drag.start.y) > 2) onCommand({ type: 'move_object', payload: { id: drag.id, dx: point.x - drag.start.x, dy: point.y - drag.start.y } }); dragRef.current = null; setDragOffset({ x: 0, y: 0 }); return; }
    finishDrawing(point);
  };
  const onWheel = (event: WheelEvent<SVGSVGElement>) => {
    event.preventDefault(); const rect = svgRef.current?.getBoundingClientRect(); if (!rect) return;
    const raw = { x: ((event.clientX - rect.left) / rect.width) * 1200, y: ((event.clientY - rect.top) / rect.height) * 760 }; const oldZoom = state.viewport.zoom; const zoom = Math.max(.55, Math.min(2.4, oldZoom + (event.deltaY > 0 ? -.08 : .08))); const boardPoint = { x: (raw.x - state.viewport.panX) / oldZoom, y: (raw.y - state.viewport.panY) / oldZoom };
    onViewport({ zoom, panX: raw.x - boardPoint.x * zoom, panY: raw.y - boardPoint.y * zoom });
  };
  const renderDraft = () => {
    if (!draft) return null;
    const { start, current, points } = draft; const stroke = activeTool === 'highlighter' ? '#d6ae50' : '#45979b'; const props = { fill: 'none', stroke, strokeWidth: activeTool === 'highlighter' ? 15 : 2.5, opacity: activeTool === 'highlighter' ? .28 : .7, strokeLinecap: 'round' as const };
